...
function applyWebsiteTheme(s){
  const mode=Number(s?.mode||0);
  const theme=!s?.running?'off':({1:'auto',2:'cool',3:'dry',4:'heat',5:'fan'}[mode]||'off');
  document.body.dataset.acTheme=theme;
  const names={off:'⏻ STANDBY',auto:'🔄 AUTO',cool:'❄️ COOL',dry:'💧 DRY',heat:'🔥 HEAT',fan:'💨 FAN'};
  const label=qs('#fan-label');
  if(label){
    const fan=s?.fan_speed!=null&&theme!=='off'? ` · Fan ${s.fan_speed}%` : '';
    label.textContent=(names[theme]||names.off)+fan;
  }
}
function renderStatus(s,{cached=false}={}){if(!s)return;currentStatus={...s};liveStatusOk=true;applyWebsiteTheme(s);qs('#status-text').textContent=cached?'Last known':'Online';qs('#status-dot').className='dot ok';const tf=cToF(s.target_temperature_c),inf=cToF(s.indoor_temperature_c),outf=cToF(s.outdoor_temperature_c);qs('#target-temp').textContent=tf??'--';qs('#indoor-temp').textContent=`Indoor ${inf??'--'}°F`;qs('#outdoor-temp').textContent=`Outdoor ${outf??'--'}°F`;const fv=qs('#fan-speed-value');if(fv)fv.textContent=s.fan_speed!=null?`${s.fan_speed}%`:'--';qs('#power-state').textContent=s.running?'AC is On':'AC is Off';qsa('.mode-tabs button').forEach(x=>x.classList.toggle('active',s.running&&String(x.dataset.mode)===String(s.mode)));const map=[['#horizontal-btn','horizontal_swing'],['#vertical-btn','vertical_swing'],['#eco-btn','eco_mode'],['#boost-btn','turbo'],['#sleep-btn','comfort_sleep']];map.forEach(([id,key])=>{const el=qs(id);if(el)el.classList.toggle('active',Boolean(s[key]));});qs('#status-detail').textContent=cached?'Showing last known state':'Live status updated.';updatePowerButtons();}

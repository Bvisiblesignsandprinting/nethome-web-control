...
def load_smart_control_config() -> dict[str, Any]:
    """Load Smart Room Control settings from the existing durable activity store."""
    if _use_postgres():
        with _pg_conn() as conn, conn.cursor() as cur:
            cur.execute(
                """
...
def save_smart_control_config(data: dict[str, Any]) -> None:
    """Persist Smart Room Control settings without exposing them in the activity feed."""
    payload = json.dumps(data, separators=(",", ":"))
    if _use_postgres():
        with _pg_conn() as conn, conn.cursor() as cur:
            cur.execute(
